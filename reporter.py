...
DRAFT_COLUMNS = ["draft_key", "competition", "week_start", "week_end", "division", "pm_name", "updated_at", "draft_json"]
...
TRANSACTIONS_HEADERS = ["transaction_id", "event_id", "transaction_date", "mission", "cycle", "event_name", "source_type", "source_name", "payment_method", "amount", "confirmed", "deposited", "deposit_date", "notes", "created_at", "updated_at"]
EXPENSES_HEADERS = ["expense_id", "event_id", "expense_date", "mission", "cycle", "event_name", "vendor", "item_description", "category", "amount", "reimbursed", "receipt_available", "notes", "created_at", "updated_at"]
GOALS_HEADERS = ["goal_id", "mission", "cycle", "goal_name", "target_amount", "deadline", "purpose", "status", "notes", "created_at", "updated_at"]
...
def append_report_to_sheet(report: dict, input_rows: list[dict], competition: str, replace_existing: bool = True) -> tuple[int, int]:
    ...
    if replace_existing:
        ...
        to_delete = []
        target_comp, target_pm, target_div, target_week = str(competition).strip().lower(), str(report.get("pm_name", "")).strip().lower(), str(report.get("division", "")).strip().lower(), str(report.get("week_start", "")).strip()
...
with st.sidebar:
    ...
    active_divisions = [d for d in DIVISIONS if any(k in d.lower() for k in ["electrical", "vehicle", "software"])] if global_mission == "Luna" else list(DIVISIONS.keys())
...
<style>
    :root {{
        --bg-top: {bg_top}; --bg-bot: {bg_bot}; --side-top: {side_top}; --side-bot: {side_bot};
        --panel: {panel_bg}; --primary: {primary}; --primary-hover: {primary_hover}; --primary-text: {primary_text};
        --primary-shadow: {primary_shadow}; --logo-a-color: {logo_a_color}; --logo-a-shadow: {logo_a_shadow};
        --logo-v-color: {logo_v_color}; --logo-v-shadow: {logo_v_shadow}; --ink: #f8fafc;
        --line: rgba(255, 255, 255, 0.15); --shadow: 0 12px 40px rgba(0, 0, 0, 0.4);
    }}
    [data-testid="stAppViewContainer"], [data-testid="stSidebar"], .hero, .glass, button, button *, .av-logo .a, .av-logo .v, div[data-baseweb="tab-highlight"], .stButton > button, [data-testid="stFormSubmitButton"] > button {{ transition: all 0.7s ease-in-out !important; }}
    [data-testid="stAppViewContainer"] {{ background: radial-gradient(circle at top, var(--bg-top) 0%, var(--bg-bot) 100%); background-attachment: fixed; color: var(--ink); }}
    [data-testid="stHeader"] {{ background: rgba(15, 23, 42, 0); }}
    [data-testid="stSidebar"] {{ background: linear-gradient(180deg, var(--side-top) 0%, var(--side-bot) 100%); border-right: 1px solid var(--line); }}
    .block-container {{ padding-top: 2rem; max-width: 1440px; }}
    .hero {{ position: relative; overflow: hidden; padding: 38px 42px; border-radius: 24px; background: var(--panel); border: 1px solid var(--line); box-shadow: var(--shadow); margin-bottom: 24px; color: #f8fafc; }}
    ...
    .glass {{ padding: 22px 28px; border-radius: 20px; background: var(--panel); border: 1px solid var(--line); box-shadow: var(--shadow); margin-bottom: 24px; color: var(--ink); }}
    div.row-widget.stRadio > div {{ display: flex; flex-direction: row; align-items: center; gap: 16px; background: rgba(255,255,255,0.05); padding: 8px 16px; border-radius: 100px; border: 1px solid var(--line); width: fit-content; }}
    .division-pill {{ display: inline-flex; align-items: center; gap: 8px; padding: 8px 12px; border-radius: 999px; background: rgba(255, 255, 255, 0.05); border: 1px solid var(--line); margin: 0 8px 8px 0; color: #f8fafc; font-size: 13px; font-weight: 750; }}
    ...
    .stButton > button {{ background: rgba(255,255,255,0.05) !important; color: #f8fafc !important; border: 1px solid rgba(255,255,255,0.1) !important; border-radius: 12px !important; font-weight: 700 !important; transition: all 0.3s ease !important; }}
    ...
    button[kind="primary"], [data-testid="stFormSubmitButton"] > button {{ background: var(--primary) !important; color: var(--primary-text) !important; box-shadow: 0 8px 20px var(--primary-shadow) !important; border: 1px solid rgba(255,255,255,0.1) !important; border-radius: 12px !important; font-weight: 800 !important; transition: all 0.3s ease !important; text-transform: uppercase; letter-spacing: 1px; }}
    ...
    .stDownloadButton > button {{ background: rgba(0,0,0,0.4) !important; color: #94a3b8 !important; border: 1px dashed rgba(255,255,255,0.2) !important; border-radius: 12px !important; font-weight: 600 !important; box-shadow: none !important; transition: all 0.3s ease !important; }}
    ...
    with st.form("new_task_form"):
        ...
        if assignee != "Unassigned" and not df_memb.empty:
            ...
            if not mem_row.empty:
                em, ro = mem_row.iloc[0].get('email', 'No email'), mem_row.iloc[0].get('role', 'No role')
                c1.caption(f"✉ {em} | ⚙ {ro}")
        ...
        if submit:
            if not title: st.error("⚠ Title is required.")
                ...
                new_task = pd.DataFrame([{
                    "task_id": str(uuid.uuid4()), "mission": mission, "cycle": cycle, "division": division, "bucket": bucket,
                    "title": title, "description": desc, "assigned_to": assignee if assignee != "Unassigned" else "",
                    "priority": priority, "status": "Not Started" if bucket != "Completed" else "Completed",
                    "due_date": due_date.strftime("%Y-%m-%d") if due_date else "", "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                }])
        ...
        if submit:
            if not title: st.error("⚠ Title is required.")
                ...
                new_row = pd.DataFrame([{
                    "content_id": str(uuid.uuid4()), "mission": mission, "cycle": cycle, "month": month_name,
                    "planned_date": p_date.strftime("%Y-%m-%d") if p_date else "", "platform": platform,
                    "content_title": title, "description": desc, "content_type": ctype, "owner": owner, "status": status,
        ...
        if submit:
            if not name: st.error("⚠ Event Name is required.")
                ...
                new_event = pd.DataFrame([{
                    "event_id": str(uuid.uuid4()), "mission": mission, "cycle": cycle, "event_name": name, "event_type": etype,
                    "planned_date": pdate.strftime("%Y-%m-%d") if pdate else "", "location": loc, "status": status,
                    "expected_gross_revenue": exp_gross, "expected_expenses": exp_costs, "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                }])
...
def create_transaction_dialog(mission, cycle, df_txns, df_events):
    ...
    with st.form("new_txn_form"):
        ...
        if st.form_submit_button("☑ Save Transaction", type="primary", use_container_width=True):
            ...
            new_t = pd.DataFrame([{
                "transaction_id": str(uuid.uuid4()), "event_id": eid, "event_name": evt if evt != "Unlinked" else "",
                "mission": mission, "cycle": cycle, "source_type": src_type, "source_name": src_name, "payment_method": method,
                "amount": amt, "transaction_date": tdate.strftime("%Y-%m-%d") if tdate else "", "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }])
...
def create_expense_dialog(mission, cycle, df_exps, df_events):
    ...
    with st.form("new_exp_form"):
        ...
        if st.form_submit_button("☑ Save Expense", type="primary", use_container_width=True):
            ...
            eid = name_to_id.get(evt, "") if evt != "Unlinked" else ""
            new_e = pd.DataFrame([{
                "expense_id": str(uuid.uuid4()), "event_id": eid, "event_name": evt if evt != "Unlinked" else "",
                "mission": mission, "cycle": cycle, "category": cat, "vendor": vendor, "item_description": item,
                "amount": amt, "expense_date": edate.strftime("%Y-%m-%d") if edate else "", "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "updated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }])
